...
class ConfWriter(ClosableObject):
    ...
    def __write_keys_val_to_file(self, append_keys, other_keys, val, defaults):
        if append_keys:
            append_keys = sorted(append_keys,
                                 ...
                                 reverse=True)
            append_keys_iter = append_keys.__iter__()
            write_keys = []
            def_val = None
            try:
                while True:
                    key = next(append_keys_iter)
                ...
                append_val = self.__get_append_val(val, def_val)
                self.__write_to_file(write_keys,
                                     append_val,
                                     self.__key_value_append_delimiter)

        if other_keys:
            self.__write_to_file(other_keys,
                                 val,
                                 self.__key_value_define_delimiter)
    ...
    def __write_to_file(self, keys, val, delimiter):
